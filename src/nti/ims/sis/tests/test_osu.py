...
class TestOSU(unittest.TestCase):
    ...
    def _parse(self, source):
        ...
        assert_that(p, 
					has_property('sourcedid', has_property('source', is_('SIS'))))
        assert_that(p, 
					has_property('sourcedid', has_property('id', is_('XXXXXXXX'))))
        ...
        assert_that(p, 
					has_property('sourcedid', has_property('source', is_('SIS'))))
        assert_that(p, 
					has_property('sourcedid', has_property('id', is_('YYYYYYYYY'))))
        ...
        assert_that(group, 
					has_property('description', is_('BIOL1311-PRIN OF BIOL LAB-WEST HTS-SEC LAB WH3')))
        assert_that(group, 
					has_property('sourcedid', has_property('source', is_('SIS'))))
        assert_that(group,
					has_property('sourcedid', has_property('id', is_('G_BIOL1311LAB_WH3_146'))))
        assert_that(group, 
					has_property('timeframe', has_property('end', is_(1420264800.0))))
        assert_that(group, 
					has_property('timeframe', has_property('start', is_(1407736800.0))))
        ...
        assert_that(member, 
					has_property('sourcedid', has_property('id', is_('XXXXXXXX'))))
        assert_that(member, 
					has_property('role', has_property('roletype', is_('02'))))
        assert_that(member, 
					has_property('role', has_property('status', is_(1))))
        assert_that(member, 
					has_property('role', has_property('userid', is_('jobs'))))
